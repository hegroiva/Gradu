run_rf_final <- function(df, features, filenamestem, language="eng", ntree=500, mtry=5, training_percent=50) {
  ...
  if ("author" %in% names(features.training)) {
    poetry_authors <- features.training$author[which(is_poetry=="POETRY")]
    ...
    poetry_authors <- features.training$varia_author[which(is_poetry=="POETRY")]
    #author <- features$author
