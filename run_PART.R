library(RWeka)
run_PART <- function(features.split, filestem="") {
  matrices = list()
  for (set_no in 1:length(features.split)) {
    print(paste0("Starting run_PART round ", set_no, " at ", date()))
	...
	#C45_control <- Weka_control(R=TRUE, M=5)
	PART_control <- Weka_control()
    PART_model <- PART(is_poetry ~ ., data=features, control=PART_control)
