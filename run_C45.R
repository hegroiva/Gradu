library(RWeka)
run_C45 <- function(features.split, filestem="") {
  matrices = list()
  for (set_no in 1:length(features.split)) {
    print(paste0("Starting run_C45 round ", set_no, " at ", date()))
	...
	C45_control <- Weka_control()
    C45_model <- J48(is_poetry ~ ., data=features, control=C45_control)
